...
async function main() {
  ...
  console.log('=== Cloudflare More Customers ===');
  const cfSlugs = ['reddit', 'medium', 'airbnb', 'uber', 'lyft', 'yelp', 'zillow', 'tripadvisor'];
  ...
  // Try Okta customer pages
  console.log('\n=== Okta Customer Pages ===');
  const oktaSlugs = ['snowflake', 'siemens', 'fedex', 'ATT', 'verizon'];
  for(const c of oktaSlugs) {
    const url = `https://www.okta.com/customers/${c}/`;
    const r = await fetch(url);
    console.log(c, ':', r.status);
    await new Promise(resolve => setTimeout(resolve, 400));
  }
