const https = require('https');
function fetch(url) {
  return new Promise((resolve, reject) => {
    https.get(url, {headers: {'User-Agent': 'Mozilla/5.0'}}, res => {
      let data = '';
      res.on('data', chunk => data += chunk);
      res.on('end', () => resolve({status: res.statusCode, body: data}));
    }).on('error', reject);
  });
}
function extractText(body) {
  return body
    .replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '')
    .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '')
    .replace(/<[^>]+>/g, ' ')
    .replace(/&nbsp;/g, ' ')
    .replace(/&amp;/g, '&')
    .replace(/&#39;/g, "'")
    .replace(/&ldquo;/g, '"')
    .replace(/&rdquo;/g, '"')
    .replace(/\s+/g, ' ')
    .trim();
}
async function main() {
  // Try more Cloudflare case studies
  console.log('=== More Cloudflare Case Studies ===');
  const cfSlugs = [
    'godaddy', 'plex', 'dropbox', 'mailchimp', 'wpengine',
    'namecheap', 'squarespace', 'wix', 'weebly', 'shopify'
  ];
  for(const c of cfSlugs) {
    const url = `https://www.cloudflare.com/case-studies/${c}/`;
    const r = await fetch(url);
    if(r.status === 200 && r.body.length > 50000) {
      const text = extractText(r.body);
      const hasMetrics = text.match(/\d+[%x]/) || text.match(/\d{3,}/);
      console.log(c, ':', r.status, 'Len:', r.body.length, hasMetrics ? '**METRICS**' : '');
      if(hasMetrics) {
        const idx = text.indexOf('We') > -1 ? text.indexOf('We') : text.indexOf('The');
        console.log('  Preview:', text.substring(idx, idx + 300));
      }
    } else {
      console.log(c, ':', r.status);
    }
    await new Promise(resolve => setTimeout(resolve, 500));
  }
  
  // Try New Relic customers index - extract names
  console.log('\n=== New Relic Customer Names ===');
  const nr = await fetch('https://newrelic.com/customers');
  const nrText = extractText(nr.body);
  // Look for customer names in the page
  const names = nrText.match(/[A-Z][a-z]+ (?:Inc|Corp|Ltd|Group|Company|Solutions)/g) || [];
  console.log('Company patterns:', [...new Set(names)].slice(0, 30));
}
main().catch(console.error);